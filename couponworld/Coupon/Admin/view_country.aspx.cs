...
namespace Coupon
{
    public partial class view_country : System.Web.UI.Page
    {
        ...
        protected void BindGrid(string query, int pageindex, string columnname = "str_country_name", string sortorder = "asc")
        {
            try
            {
                int totalcount;
                ...
                dt = objbal.country_Select(query, pageindex, appConstant.pageSize, out totalcount, sortorder, columnname);
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    query = " where str_country_name like '%" + txtsearch.Value + "%' OR str_region like '%" + txtsearch.Value + "%'";
                }
                BindGrid(query, 1);
            }
            catch (Exception ex)
        ...
        protected void lnk_del_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                tbl_country_bal objbal = new tbl_country_bal();
                tbl_country_entity objentity = new tbl_country_entity();
                ...
                for (int i = 0; i < rpview.Items.Count; i++)
                {
                    ...
                    {
                        objentity.int_country_id = Convert.ToInt16(hdnID.Value);
                        objbal.country_delete(objentity);
                    }
                }
        ...
        {
            Response.Redirect("edit_country.aspx");
        }
