...
{
    public partial class view_organization : System.Web.UI.Page
    {
        ...
        {
            Response.Redirect("edit_organization.aspx");
        }
        ...
        protected void lnk_deactive_Click(object sender, EventArgs e)
        {
            try
            {
                LinkButton lnk = (LinkButton)sender;
                ...
                objentity.int_organization_id = Convert.ToInt64(lnk.CommandArgument);
                objentity.bit_spotlight = false;

                int val = objbal.organization_spotlight_deactive(objentity);
                if (val > 0)
                {
                    BindGrid("", 1);
                }
        ...
        protected void lnk_spotlight_Click(object sender, EventArgs e)
        {
            try
            {
                LinkButton lnk = (LinkButton)sender;
                ...
                objentity.int_organization_id = Convert.ToInt64(lnk.CommandArgument);
                objentity.bit_spotlight = true;

                int val = objbal.organization_spotlight_Update(objentity);
                if (val > 0)
                {
                    BindGrid("", 1);
                }
