...
{
    public partial class view_connect_category : System.Web.UI.Page
    {
        ...
        protected void BindGrid(string query, int pageindex, string columnname = "str_cat", string sortorder = "asc")
        {
            try
            {
                ...
                dt = objbal.cat_select(query, pageindex, appConstant.pageSize, out totalcount, sortorder, columnname);
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    query = " where str_cat like '%" + txtsearch.Value + "%'";
                }
                BindGrid(query, 1);
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                for (int i = 0; i < rpview.Items.Count; i++)
                {
                    ...
                    if (chkDelete.Checked)
                    {
                        ...
                        objentity.int_connect_cat_id = Convert.ToInt32(hdnID.Value);
                        objbal.cat_delete(objentity);
                    }
                }
                BindGrid("", 1);
            }
        ...
        {
            Response.Redirect("edit_connect_category.aspx");
        }
