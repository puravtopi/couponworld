...
{
    public partial class view_audit_list : System.Web.UI.Page
    {
        ...
        protected void BindGrid(string query, int pageindex, string columnname = "str_first_name", string sortorder = "asc")
        {
            try
            {
                ...
                dt = objbal.auditmaster_Select(query, pageindex, appConstant.pageSize, out totalcount, sortorder, columnname);
        ...
        protected void lnk_del_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < rpview.Items.Count; i++)
                {
                    ...
                    {
                        objentity.int_log_id = Convert.ToInt16(hdnID.Value);
                        objbal.auditmaster_delete(objentity);
                    }
                }
                BindGrid("", 1);
            }
        ...
        {
            Response.Redirect("edit_audit.aspx");
        }

        protected void lnk_report_name_Click(object sender, EventArgs e)
        {
            LinkButton lnk = (LinkButton)sender;
