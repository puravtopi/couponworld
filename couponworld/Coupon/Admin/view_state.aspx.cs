...
namespace Coupon.Admin
{
    public partial class view_state : System.Web.UI.Page
    {
        ...
        protected void BindGrid(string query, int pageindex, string columnname = "str_state_name", string sortorder = "asc")
        {
            try
            {
                int totalcount;
                ...
                dt = objbal.state_Select(query, pageindex, 10, out totalcount, sortorder, columnname);
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    query = " where str_state_name like '%" + txtsearch.Value + "%' OR str_country_name like '%" + txtsearch.Value + "%'";
                }
                BindGrid(query, 1);
            }
            catch (Exception ex)
        ...
        protected void lnk_del_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                tbl_state_bal objbal = new tbl_state_bal();
                tbl_state_entity objentity = new tbl_state_entity();
                ...
                for (int i = 0; i < rpview.Items.Count; i++)
                {
                    ...
                    {
                        objentity.int_state_id = Convert.ToInt16(hdnID.Value);
                        objbal.state_delete(objentity);
                    }
                }
        ...
        {
            Response.Redirect("edit_state.aspx");
        }
        ...
        protected void lnk_state_name_Click(object sender, EventArgs e)
        {
            LinkButton lnk = (LinkButton)sender;
