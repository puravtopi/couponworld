...
{
    public partial class view_discount_type : System.Web.UI.Page
    {
        ...
        protected void BindGrid(string query, int pageindex, string columnname = "str_discount_type", string sortorder = "asc")
        {
            try
            {
                int totalcount;
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    query = " where str_discount_type like '%" + txtsearch.Value + "%'";
                }
                BindGrid(query, 1);
            }
            catch (Exception ex)
        ...
        protected void lnk_del_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < rpview.Items.Count; i++)
                {
                    ...
                    {
                        objentity.int_discount_type_id = Convert.ToInt16(hdnID.Value);
                        objbal.discount_type_delete(objentity);
                    }
                }
                BindGrid("", 1);
            }
        ...
        {
            Response.Redirect("edit_document_type.aspx");
        }

        protected void lnk_dic_type_Click(object sender, EventArgs e)
        {
            LinkButton lnk = (LinkButton)sender;
