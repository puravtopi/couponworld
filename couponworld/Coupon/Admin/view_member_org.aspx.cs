...
{
    public partial class view_member_org : System.Web.UI.Page
    {
        ...
        protected void BindGrid(string query, int pageindex, string columnname = "str_first_name", string sortorder = "asc")
        {
            try
            {
                ...
                tbl_member_organization_bal objbal = new tbl_member_organization_bal();

                dt = objbal.member_organization_Select(query, pageindex, 10, out totalcount, sortorder, columnname);
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    query = " where str_first_name like '%" + txtsearch.Value + "%'";
                }
                BindGrid(query, 1);
            }
            catch (Exception ex)
        ...
        protected void lnk_del_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                tbl_member_organization_bal objbal = new tbl_member_organization_bal();
                tbl_member_organization_entity objentity = new tbl_member_organization_entity();
                ...
                for (int i = 0; i < rpview.Items.Count; i++)
                {
                    ...
                    {
                        objentity.int_member_organization_id = Convert.ToInt16(hdnID.Value);
                        objbal.member_organization_delete(objentity);
                    }
                }
                BindGrid("", 1);
            }
        ...
        protected void lnk__member_org_name_Click(object sender, EventArgs e)
        {
            LinkButton lnk = (LinkButton)sender;
