    ...
    public partial class view_group : System.Web.UI.Page
    {
        ...
        protected void BindGrid(string query, int pageindex, string columnname = "str_name", string sortorder = "asc")
        {
            try
            {
                ...
                dt = objbal.group_Select(query, pageindex, 10, out totalcount, sortorder, columnname);
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    query = " where str_name like '%" + txtsearch.Value + "%' ";
                }
                BindGrid(query, 1);
            }
            catch (Exception ex)
        ...
        protected void lnk_del_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < rpview.Items.Count; i++)
                {
                    ...
                    {
                        objentity.int_group_id = Convert.ToInt16(hdnID.Value);
                        objbal.group_delete(objentity);
                    }
                }
                BindGrid("", 1);
            }
        ...
        {
            Response.Redirect("edit_group.aspx");
        }

        protected void lnk_grp_name_Click(object sender, EventArgs e)
        {
            LinkButton lnk = (LinkButton)sender;
