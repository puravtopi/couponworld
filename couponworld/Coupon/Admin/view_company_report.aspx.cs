...
{
    public partial class view_company_report : System.Web.UI.Page
    {
        ...
        protected void ddl_company_type_bind()
        {
            try
            {
                ...
                ddl_type.DataValueField = "int_company_type_id";
                ddl_type.DataTextField = "str_company_type";
        ...
        protected void BindGrid(string query, int pageindex, string columnname = "str_company_name", string sortorder = "asc")
        {
            try
            {
                ...
                dt = objbal.company_Select(query, pageindex, appConstant.pageSize, out totalcount, sortorder, columnname);
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (string.IsNullOrEmpty(cnd))
                        cnd = " where int_company_type_id=" + ddl_type.SelectedItem.Value;
                    else
                        cnd = cnd + " " + rbloption.SelectedItem.Text + " int_company_type_id=" + ddl_type.SelectedItem.Value;
                }
                ...
                if (!string.IsNullOrEmpty(txt_member_name.Value))
                {
                    if (string.IsNullOrEmpty(cnd))
                        cnd = "where str_company_name like '%" + txt_member_name.Value + "%' ";
                    else
                        cnd = cnd + " " + rbloption.SelectedItem.Text + " str_company_name like '%" + txt_member_name.Value + "%'";
                }
                if (!string.IsNullOrEmpty(txt_member_no.Value))
                {
                    if (string.IsNullOrEmpty(cnd))
                        cnd = "where int_company_id = '" + txt_member_no.Value + "' ";
                    else
                        cnd = cnd + " " + rbloption.SelectedItem.Text + " int_company_id = '" + txt_member_no.Value + "'";
                }

                //string query = "";
                //if (!string.IsNullOrEmpty(txtsearch.Value))
                //{
                //    query = " where str_company_name like '%" + txtsearch.Value + "%' OR str_corporate_name like '%" + txtsearch.Value + "%' OR str_short_name like '%" + txtsearch.Value + "%'  OR str_city_name like '%" + txtsearch.Value + "%' OR int_company_id like '%" + txtsearch.Value + "%' OR str_email_id_p like '%" + txtsearch.Value + "%' OR str_contact_1_p like '%" + txtsearch.Value + "%'";
                //}
                //ViewState["cnd"] = query;
                BindGrid(cnd, 1);
            }
        ...
        protected void lnk_del_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < rpview.Items.Count; i++)
                {
                    ...
                    {
                        objentity.int_company_id = Convert.ToInt16(hdnID.Value);
                        objbal.company_delete(objentity);
                    }
                }
                BindGrid("", 1);
            }
        ...
        protected void btn_export_excel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataSet dt = objbal.company_Select(ViewState["cnd"].ToString(), 1, 0, out totalcount, "asc", "str_company_name");
                if (dt.Tables[0].Rows.Count > 0)
                {
                ...
                ExportGridToExcel(dt.Tables[0], "company_" + System.DateTime.Now.ToString());
            }
            catch (Exception ex)
            {
            }
        ...
        protected void btn_pdf_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ExportToPDF(dt.Tables[0], "company_" + System.DateTime.Now.ToString());
            }
            catch (Exception ex)
            {
            }
