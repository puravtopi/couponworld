...
{
    public partial class view_organization_report : System.Web.UI.Page
    {
        ...
        protected void ddl_org_type_bind()
        {
            try
            {
                ...
                ddl_type.Items.Insert(0, new ListItem("-- Search by Organization Type --", "0"));
            }
            catch (Exception ex)
            {
            }
        ...
        protected void BindGrid(string query, int pageindex, string columnname = "str_organization_name", string sortorder = "asc")
        {
            try
            {
                ...
                dt = objbal.organization_Select(query, pageindex, appConstant.pageSize, out totalcount, sortorder, columnname);
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (string.IsNullOrEmpty(cnd))
                        cnd = "where int_organization_type_id='" + ddl_type.SelectedItem.Value + "' ";
                    else
                        cnd = cnd + "and int_organization_type_id='" + ddl_type.SelectedItem.Value + "'  ";
                }
                if (!string.IsNullOrEmpty(txt_zipcode.Value))
                {
                    if (string.IsNullOrEmpty(cnd))
                        cnd = "where int_zip_code = '" + txt_zipcode.Value + "' ";
                    else
                        cnd = cnd + " " + rbloption.SelectedItem.Text + " int_zip_code = '" + txt_zipcode.Value + "'";
                }
                if (!string.IsNullOrEmpty(txt_org_name.Value))
                {
                    if (string.IsNullOrEmpty(cnd))
                        cnd = "where str_organization_name like '%" + txt_org_name.Value + "%' ";
                    else
                        cnd = cnd + " " + rbloption.SelectedItem.Text + " str_organization_name like '%" + txt_org_name.Value + "%'";
                }
                if (!string.IsNullOrEmpty(txt_org_no.Value))
                {
                    if (string.IsNullOrEmpty(cnd))
                        cnd = "where int_organization_id = '" + txt_org_no.Value + "' ";
                    else
                        cnd = cnd + " " + rbloption.SelectedItem.Text + " int_organization_id = '" + txt_org_no.Value + "'";
                }
                //string query = "";
                //if (!string.IsNullOrEmpty(txtsearch.Value))
                //{
                //    query = " where str_organization_name like '%" + txtsearch.Value + "%' OR str_corporate_name like '%" + txtsearch.Value + "%' OR str_short_name like '%" + txtsearch.Value + "%'  OR str_city_name like '%" + txtsearch.Value + "%' OR int_organization_id like '%" + txtsearch.Value + "%' OR str_email_id_p like '%" + txtsearch.Value + "%' OR str_contact_1_p like '%" + txtsearch.Value + "%'";
                //}
                //ViewState["cnd"] = query;
                BindGrid(cnd, 1);
            }
        ...
        protected void lnk_del_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < rpview.Items.Count; i++)
                {
                    ...
                    {
                        objentity.int_organization_id = Convert.ToInt16(hdnID.Value);
                        objbal.organization_delete(objentity);
                    }
                }
                BindGrid("", 1);
            }
        ...
        {
            Response.Redirect("organization_details.aspx");
        }

        protected void lnk_org_name_Click(object sender, EventArgs e)
        {
            LinkButton lnk = (LinkButton)sender;
        ...
        protected void btn_export_excel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataSet dt = objbal.organization_Select(ViewState["cnd"].ToString(), 1, 0, out totalcount, "asc", "str_organization_name");
                if (dt.Tables[0].Rows.Count > 0)
                {
                ...
                ExportGridToExcel(dt.Tables[0], "organization_" + System.DateTime.Now.ToString());
            }
            catch (Exception ex)
            {
            }
        ...
        protected void btn_pdf_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ExportToPDF(dt.Tables[0], "organization_" + System.DateTime.Now.ToString());
            }
            catch (Exception ex)
            {
            }
