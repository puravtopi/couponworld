...
namespace Coupon.Admin
{
    public partial class view_category : System.Web.UI.Page
    {
        ...
        protected void BindGrid(string query, int pageindex, string columnname = "str_category_name", string sortorder = "asc")
        {
            try
            {
                int totalcount;
                ...
                dt = objbal.category_Select(query, pageindex, appConstant.pageSize, out totalcount, sortorder, columnname);
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    query = " where str_category_name like '%" + txtsearch.Value + "%' ";
                }
                BindGrid(query, 1);
            }
            catch (Exception ex)
        ...
        protected void lnk_del_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                tbl_category_bal objbal = new tbl_category_bal();
                tbl_category_entity objentity = new tbl_category_entity();
                ...
                for (int i = 0; i < rpview.Items.Count; i++)
                {
                    ...
                    {
                        objentity.int_category_id = Convert.ToInt16(hdnID.Value);
                        objbal.category_delete(objentity);
                    }
                }
