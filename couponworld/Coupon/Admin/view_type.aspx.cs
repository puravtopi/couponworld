...
{
    public partial class view_type : System.Web.UI.Page
    {
        ...
        protected void BindGrid(string query, int pageindex, string columnname = "str_type_name", string sortorder = "asc")
        {
            try
            {
                int totalcount;
                ...
                dt = objbal.types_Select(query, pageindex, 10, out totalcount, sortorder, columnname);
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    query = " where str_type_name like '%" + txtsearch.Value + "%' ";
                }
                BindGrid(query, 1);
            }
            catch (Exception ex)
        ...
        protected void lnk_del_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                tbl_types_bal objbal = new tbl_types_bal();
                tbl_types_entity objentity = new tbl_types_entity();
                ...
                for (int i = 0; i < rpview.Items.Count; i++)
                {
                    ...
                    {
                        objentity.int_type_id = Convert.ToInt16(hdnID.Value);
                        objbal.types_delete(objentity);
                    }
                }
                BindGrid("", 1);
            }
