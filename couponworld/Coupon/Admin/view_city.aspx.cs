...
namespace Coupon.Admin
{
    public partial class view_city : System.Web.UI.Page
    {
        ...
        protected void BindGrid(string query, int pageindex, string columnname = "str_city_name", string sortorder = "asc")
        {
            try
            {
                int totalcount;
                ...
                dt = objbal.city_Select(query, pageindex, appConstant.pageSize, out totalcount, sortorder, columnname);
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    query = " where str_state_name like '%" + txtsearch.Value + "%' OR str_city_name like '%" + txtsearch.Value + "%'";
                }
                BindGrid(query, 1);
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                for (int i = 0; i < rpview.Items.Count; i++)
                {
                    ...
                    if (chkDelete.Checked)
                    {
                        ...
                        objentity.int_city_id = Convert.ToInt16(hdnID.Value);
                        objbal.city_delete(objentity);
                    }
                }
                BindGrid("", 1);
            }
        ...
        {
            Response.Redirect("edit_city.aspx");
        }
