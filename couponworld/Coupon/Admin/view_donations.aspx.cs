...
{
    public partial class view_donations : System.Web.UI.Page
    {
        ...
        protected void BindGrid(string query, int pageindex, string columnname = "str_name", string sortorder = "asc")
        {
            try
            {
                ...
                dt = objbal.donation_Select(query, pageindex, appConstant.pageSize, out totalcount, sortorder, columnname);
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (string.IsNullOrEmpty(cnd))
                        cnd = "where str_name like '%" + txtsearch.Value + "%' ";
                    else
                        cnd = cnd + " " + rbloption.SelectedItem.Text + " str_name like '%" + txtsearch.Value + "%'";
                }
        ...
        protected void lnk_del_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < rpview.Items.Count; i++)
                {
                    ...
                    {
                        objentity.int_donation_id = Convert.ToInt16(hdnID.Value);
                        objbal.donation_delete(objentity);
                    }
                }
                BindGrid("", 1);
            }
        ...
        protected void lnk_org_donation_Click(object sender, EventArgs e)
        {
            LinkButton lnk = (LinkButton)sender;
        ...
        protected void btn_export_excel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataSet dt = objbal.donation_Select(ViewState["cnd"].ToString(), 1, 0, out totalcount, "asc", "str_name");
                if (dt.Tables[0].Rows.Count > 0)
                {
                    rpview.DataSource = dt;
                    rpview.DataBind();
                ...
                ExportGridToExcel(dt.Tables[0], "donation_" + System.DateTime.Now.ToString());
            }
            catch (Exception ex)
            {
            }
        ...
        protected void btn_pdf_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ExportToPDF(dt.Tables[0], "donation_" + System.DateTime.Now.ToString());
            }
            catch (Exception ex)
            {
            }
