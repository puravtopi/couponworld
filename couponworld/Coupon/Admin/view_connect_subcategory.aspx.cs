...
{
    public partial class view_connect_subcategory : System.Web.UI.Page
    {
        ...
        protected void BindGrid(string query, int pageindex, string columnname = "str_cat", string sortorder = "asc")
        {
            try
            {
                int totalcount;
                ...
                dt = objbal.subcat_select(query, pageindex, appConstant.pageSize, out totalcount, sortorder, columnname);
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    query = " where str_cat like '%" + txtsearch.Value + "%' and str_sub_cat like '%" + txtsearch.Value + "'";
                }
                BindGrid(query, 1);
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                for (int i = 0; i < rpview.Items.Count; i++)
                {
                    ...
                    if (chkDelete.Checked)
                    {
                        ...
                        objentity.int_connect_sub_cat_id = Convert.ToInt32(hdnID.Value);
                        objbal.subcat_delete(objentity);
                    }
                }
                BindGrid("", 1);
            }
        ...
        {
            Response.Redirect("edit_connect_subcategory.aspx");
        }
        ...
        protected void lnk_cat_name_Click(object sender, EventArgs e)
        {
            LinkButton lnk = (LinkButton)sender;
