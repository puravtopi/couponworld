...
{
    public partial class view_company_type : System.Web.UI.Page
    {
        ...
        protected void BindGrid(string query, int pageindex, string columnname = "str_company_type", string sortorder = "asc")
        {
            try
            {
                int totalcount;
                ...
                dt = objbal.company_type_Select(query, pageindex, 10, out totalcount, sortorder, columnname);
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    query = " where str_company_type like '%" + txtsearch.Value + "%' ";
                }
                BindGrid(query, 1);
            }
            catch (Exception ex)
        ...
        protected void lnk_del_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                tbl_company_type_bal objbal = new tbl_company_type_bal();
                tbl_company_type_entity objentity = new tbl_company_type_entity();
                ...
                for (int i = 0; i < rpview.Items.Count; i++)
                {
                    ...
                    {
                        objentity.int_company_type_id = Convert.ToInt16(hdnID.Value);
                        objbal.company_types_delete(objentity);
                    }
                }
                BindGrid("", 1);
            }
        ...
        {
            Response.Redirect("edit_company_type.aspx");
        }
