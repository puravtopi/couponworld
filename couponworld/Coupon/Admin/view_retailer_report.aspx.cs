...
{
    public partial class view_retailer_report : System.Web.UI.Page
    {
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(txt_retailer_name.Value))
                {
                    if (string.IsNullOrEmpty(cnd))
                        cnd = "where str_retailer_name like '%" + txt_retailer_name.Value + "%' ";
                    else
                        cnd = cnd + " " + rbloption.SelectedItem.Text + " str_retailer_name like '%" + txt_retailer_name.Value + "%'";
                }
                if (!string.IsNullOrEmpty(txt_retailer_no.Value))
                {
                    if (string.IsNullOrEmpty(cnd))
                        cnd = "where int_retailer_id = '" + txt_retailer_no.Value + "' ";
                    else
                        cnd = cnd + " " + rbloption.SelectedItem.Text + " int_retailer_id = '" + txt_retailer_no.Value + "'";
                }
                //string query = "";
                //if (!string.IsNullOrEmpty(txtsearch.Value))
                //{
                //    query = " where str_retailer_name like '%" + txtsearch.Value + "%' OR str_corporate_name like '%" + txtsearch.Value + "%' OR str_short_name like '%" + txtsearch.Value + "%'OR int_retailer_id like '%" + txtsearch.Value + "%'";
                //}
                BindGrid(cnd, 1);
            }
            catch (Exception ex)
        ...
        protected void btn_export_excel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataSet dt = objbal.retailer_Select(ViewState["cnd"].ToString(), 1, 0, out totalcount, "asc", "str_retailer_name");
                if (dt.Tables[0].Rows.Count > 0)
                {
                ...
                ExportGridToExcel(dt.Tables[0], "retailer_" + System.DateTime.Now.ToString());
            }
            catch (Exception ex)
            {
            }
        ...
        protected void btn_pdf_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ExportToPDF(dt.Tables[0], "retailer_" + System.DateTime.Now.ToString());
            }
            catch (Exception ex)
            {
            }
