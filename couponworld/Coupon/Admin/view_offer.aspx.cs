    ...
    public partial class view_offer : System.Web.UI.Page
    {
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    query = " where str_offer_name like '%" + txtsearch.Value + "%'";
                }
                BindGrid(query, 1);
            }
            catch (Exception ex)
        ...
        {
            Response.Redirect("edit_offer.aspx");
        }
        ...
        protected void lbl_active_Click(object sender, EventArgs e)
        {
            try
            {
                LinkButton lnk = (LinkButton)sender;
                ...
                objentity.int_offer_id = Convert.ToInt64(lnk.CommandArgument);
                objentity.bit_accept = true;

                int val = objbal.retailer_status_Update(objentity);
                if (val > 0)
                {
                    BindGrid("", 1);
                }
        ...
        protected void lbl_deactive_Click(object sender, EventArgs e)
        {
            try
            {
                LinkButton lnk = (LinkButton)sender;
                ...
                int val = objbal.retailer_status_Update_deactive(objentity);
                if (val > 0)
                {
                    BindGrid("", 1);
                }
