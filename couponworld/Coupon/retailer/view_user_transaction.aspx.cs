...
namespace Coupon
{
    public partial class view_user_transaction : System.Web.UI.Page
    {
        ...
        protected void BindGrid(string query, int pageindex, string columnname = "str_first_name", string sortorder = "asc")
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(query))
                    query = " where int_retailer_id = " + Session["rid"];
                else
                    query = query + " and int_retailer_id = " + Session["rid"];
        ...
        protected void btn_export_excel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataSet dt = objbal.transaction_Select(" where int_retailer_id = " + Session["rid"], 1, 0, out totalcount, "asc", "str_first_name");
                if (dt.Tables[0].Rows.Count > 0)
                {
                    rpview.DataSource = dt;
                    rpview.DataBind();
